...
namespace FontStashSharp
{
  public class FontSystem : ITextShapingInfoProvider, IDisposable
  {
...
#if RAYLIB
		  Raylib.UnloadTexture(atlas.Texture.Value); // TODO: not sure
#else
...
#endif
	{
	  ...
	  var rectEmpty = ExistingTextureUsedSpace.Height == 0 && ExistingTextureUsedSpace.Width == 0 && ExistingTextureUsedSpace.X == 0 && ExistingTextureUsedSpace.Y == 0;

	  if (existingTexture != null && !rectEmpty)
	  {
		if (!fontAtlas.AddSkylineLevel(0, (int)ExistingTextureUsedSpace.X, (int)ExistingTextureUsedSpace.Y, (int)ExistingTextureUsedSpace.Width, (int)ExistingTextureUsedSpace.Height))
		{
...
#endif
	{
...
#else
		textureSize = device.GetTextureSize(ExistingTexture.Value);
#endif
	  }
	  ...
	  {
		SetFontAtlas(CreateFontAtlas(device, (int)textureSize.X, (int)textureSize.Y));
	  }
	  var atlas = CurrentAtlas;
	  if (!atlas.AddRect((int)gw, (int)gh, ref gx, ref gy))
	  {
