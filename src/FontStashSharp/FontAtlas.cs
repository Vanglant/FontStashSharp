...
namespace FontStashSharp
{
  public class FontAtlas
  {
...
#endif
	{
	  ...
	  {
		colorBuffer = new byte[(int)colorBufferSize];
	  ...
	  Array.Clear(colorBuffer, 0, (int)colorBufferSize);
...
#else
	  textureManager.SetTextureData(Texture.Value, eraseArea, colorBuffer);
#endif

	  fontSource.RasterizeGlyphBitmap(glyph.Id,
		  glyph.FontSize,
		  buffer,
		  glyph.EffectAmount + glyph.EffectAmount * (int)glyph.Size.X,
		  (int)glyph.Size.X - glyph.EffectAmount * 2,
		  (int)glyph.Size.Y - glyph.EffectAmount * 2,
		  (int)glyph.Size.X);
...
#else
	  textureManager.SetTextureData(Texture.Value, glyph.TextureRectangle, colorBuffer);
#endif
	}
  }
}